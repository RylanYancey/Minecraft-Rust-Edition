    ...
    for (interaction, action, mut image, mut last, children, toggle, images) in &mut query {
        let mut set_text_color = |to: Color| {
            if let Ok(mut text) = child_text.get_mut(children[0]) {
                text.sections
                    .iter_mut()
                    .for_each(|section| section.style.color = to);
            }
        };
        ...
        match *interaction {
            Interaction::Pressed => {
                image.texture = images.disabled.clone();
                (set_text_color)(Color::srgb(0.5, 0.5, 0.5));
            }

            Interaction::Hovered => {
                image.texture = images.hovered.clone();
            ...
            Interaction::None => {
                (set_text_color)(Color::WHITE);
                image.texture = images.button.clone();
            }
        }
...
pub fn handle_button_toggle(
    mut query: Query<
        (&Toggled<Button>, &mut UiImage, &Children, &ButtonImages), 
